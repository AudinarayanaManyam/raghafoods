...
const shareUtils = {
    generateShareUrl: (platform, title, text, url)=>{
        switch(platform){
            ...
            case 'twitter':
                return "https://twitter.com/intent/tweet?text=".concat(encodeURIComponent(text), "&url=").concat(encodeURIComponent(url));
            case 'whatsapp':
                return "https://wa.me/?text=".concat(encodeURIComponent(text + ' ' + url));
            case 'linkedin':
                return "https://www.linkedin.com/sharing/share-offsite/?url=".concat(encodeURIComponent(url));
            case 'email':
                return "mailto:?subject=".concat(encodeURIComponent(title), "&body=").concat(encodeURIComponent(text + '\n\n' + url));
            default:
                return '';
        }
    },
...
const PromoCodes = ()=>{
    ...
    const handleShare = (promo, platform)=>{
        const promoText = "Check out this amazing offer from Raaga Foods: ".concat(promo.title, ' - Use code "').concat(promo.code, '" for ').concat(promo.discount, "! Valid until ").concat(promo.validUntil);
        const promoUrl = "".concat(window.location.origin, "#promo-").concat(promo.id);
        const title = "Raaga Foods Offer - ".concat(promo.title);
        __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$components$2f$ShareMenu$2e$tsx__$5b$app$2d$client$5d$__$28$ecmascript$29$__["shareUtils"].handleShare(platform, title, promoText, promoUrl, ()=>{
            setOpenShareMenus((prev)=>({
                    ...prev,
                    [promo.id]: false
                }));
...
//# sourceMappingURL=src_69f5bf6f._.js.map